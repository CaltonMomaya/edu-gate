...
export default function TeachersPage() {
  ...
  async function toggleActive(staffMember: StaffMember) {
    ...
    if (!confirm(`${action.charAt(0).toUpperCase() + action.slice(1)} ${staffMember.full_name}?`)) return;
        ...
        <Card className="border-0 shadow-sm lg:col-span-1">
          ...
          <CardContent>
            <form onSubmit={createStaffMember} className="space-y-4">
              <div className="space-y-2"><Label>Full Name *</Label><Input value={fullName} onChange={e => setFullName(e.target.value)} placeholder="e.g., Jane Muthoni" required /></div>
        ...
        <Card className="border-0 shadow-sm lg:col-span-2">
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {staff.length === 0 ? (
                  <TableRow><TableCell colSpan={5} className="text-center text-slate-500 py-8">No staff yet.</TableCell></TableRow>
                ) : staff.map(m => (
                  <TableRow key={m.id} className={!m.is_active ? 'opacity-50' : ''}>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" onClick={() => openEdit(m)}><Edit className="h-4 w-4 text-blue-600" /></Button>
                        <Button variant="ghost" size="icon" onClick={() => toggleActive(m)}>
                          {m.is_active ? <Ban className="h-4 w-4 text-red-600" /> : <CheckCircle className="h-4 w-4 text-emerald-600" />}
                        </Button>
                      </div>
                    </TableCell>
                  </TableRow>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2"><Label>Role</Label>
              <Select value={editRole} onValueChange={setEditRole}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{ROLES.map(r => <SelectItem key={r.value} value={r.value}>{r.label}</SelectItem>)}</SelectContent></Select>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditOpen(false)}>Cancel</Button>
            <Button onClick={saveEdit} disabled={isLoading}>{isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : 'Save'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
